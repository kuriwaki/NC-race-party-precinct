...
[config/pipeline.yml](../../config/pipeline.yml) selects
`universities.mode: reviewed_points`. The upstream college shapefile, R1
workbook, and [alternative crosswalk](../../references/r1-universities-crosswalk.yml)
are not required for this mode. The optional `reviewed_crosswalk` mode would
require a separate completed crosswalk and a reviewed input-manifest update.
